...
async function handleSend() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const summaryParts = [];
      summaryParts.push(`I detected **${data.customer_emotion}** emotion with **${data.sentiment}** sentiment (${data.confidence.toFixed(0)}% confidence).`);
      if (data.main_issue !== "N/A") summaryParts.push(`The main issue appears to be: **${data.main_issue}**.`);
